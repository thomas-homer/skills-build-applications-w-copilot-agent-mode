...
3. Ensure `/` points to the api and `api_root` is present in `urls.py`.Let's add the following to a prompt file called `update-octofit-tracker-app.prompt.md` in the `.github/prompts` directory and add mode: 'agent' and model: GPT-4.1 to the prompt file.
...
SECRET_KEY = 'django-insecure-)ybrt+a*)d&btevl!h=@adq-7+^ogwshzs^_$6tlaj2u$i4tv*'
